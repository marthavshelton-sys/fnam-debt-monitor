...
const cal = load("calendar.json", { nextResults: null });
emit("reference.js", "ORCL_REF", {
  updatedAt: now.slice(0, 10),
  // Next results date once Oracle announces it ({ date, source }); null until then, and the page/presentation assume it from the release history.
  calendar: { nextResults: cal.nextResults && cal.nextResults.date ? cal.nextResults : null },
...
const calAll = load("calendar.json", null);
if (calAll) emit("calendar.js", "ORCL_CALENDAR", { generated: calAll.generated, pastMonths: calAll.past_months, sources: calAll.sources, events: calAll.events, estimates: calAll.estimates, manualEvents: calAll.manual_events, nextResults: calAll.nextResults || null }, "Oracle investor calendar — earnings calls, analyst days and conferences from the IR events page and date-setting releases; estimated windows are labelled derived (tools/oracle/data/calendar.json).");
