...
| `test-parsers.mjs` | after every build | every archived 8-K exhibit in `tools/oracle/raw/8k/` | fails when the printed "Total revenues" / "Total operating expenses" no longer match `quarters.json` (release format changed) |
