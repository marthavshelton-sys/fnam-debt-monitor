// Password gate for https://fnam.mx/asur/* — a Cloudflare Pages Function middleware.
//
// Every request under /asur/ (the page, its data files, everything) passes through here before
...
//   ASUR_PASSWORD        the shared password. While it is UNSET the gate is dormant: /asur/ is served
...
//   ASUR_SESSION_SECRET  optional. Random string used to sign session cookies. Defaults to a
...
//   ASUR_SESSION_DAYS    optional. Session length in days (default 30). "?logout" ends a session.
//
...
async function sessionSecret(env) {
  return env.ASUR_SESSION_SECRET || (await sha256Hex('asur-session:' + env.ASUR_PASSWORD));
}
async function makeSession(env) {
  const days = Number(env.ASUR_SESSION_DAYS) > 0 ? Number(env.ASUR_SESSION_DAYS) : 30;
...
function cookieHeader(value, maxAge) {
  return `${COOKIE}=${value}; Path=/asur; Max-Age=${maxAge}; HttpOnly; Secure; SameSite=Lax`;
}
...
<p class="fine">Datos públicos (informes trimestrales, 20-F y comunicados de ASUR, BMV/SEC). No contiene información privilegiada.</p>`,
  });
}

export async function onRequest({ request, env, next }) {
  const url = new URL(request.url);
  if (!env.ASUR_PASSWORD) {
    // Password protection not enabled yet: serve the page, keep it out of search engines and caches.
    if (request.method === 'POST' && url.pathname === LOGIN_PATH) return new Response(null, { status: 303, headers: { location: '/asur/' } });
  ...
  if (url.searchParams.has('logout')) {
    return new Response(null, { status: 303, headers: { location: '/asur/', 'set-cookie': cookieHeader('', 0), 'cache-control': 'no-store' } });
  }

  if (request.method === 'POST' && url.pathname === LOGIN_PATH) {
    let password = '';
    try { password = String((await request.formData()).get('password') || ''); } catch { password = ''; }
    if (password && timingSafeEqual(password, env.ASUR_PASSWORD)) {
      const s = await makeSession(env);
      return new Response(null, { status: 303, headers: { location: '/asur/', 'set-cookie': cookieHeader(encodeURIComponent(s.value), s.maxAge), 'cache-control': 'no-store' } });
    }
