// Password gate for https://fnam.mx/gap/* — a Cloudflare Pages Function middleware.
//
// Every request under /gap/ (the page, its data files, everything) passes through here before
...
//   GAP_SESSION_DAYS    optional. Session length in days (default 30). "?logout" ends a session.
//
...
async function sessionSecret(env) {
  return env.GAP_SESSION_SECRET || (await sha256Hex('gap-session:' + env.GAP_PASSWORD));
}
async function makeSession(env) {
  const days = Number(env.GAP_SESSION_DAYS) > 0 ? Number(env.GAP_SESSION_DAYS) : 30;
...
function cookieHeader(value, maxAge) {
  return `${COOKIE}=${value}; Path=/gap; Max-Age=${maxAge}; HttpOnly; Secure; SameSite=Lax`;
}
...
<p class="fine">Datos públicos (informes trimestrales, 20-F y comunicados de GAP, BMV/SEC). No contiene información privilegiada.</p>`,
  });
}
...
export async function onRequest({ request, env, next }) {
  ...
  if (url.searchParams.has('logout')) {
    return new Response(null, { status: 303, headers: { location: '/gap/', 'set-cookie': cookieHeader('', 0), 'cache-control': 'no-store' } });
  }

  if (request.method === 'POST' && url.pathname === LOGIN_PATH) {
    let password = '';
    try { password = String((await request.formData()).get('password') || ''); } catch { password = ''; }
    if (password && timingSafeEqual(password, env.GAP_PASSWORD)) {
      const s = await makeSession(env);
      return new Response(null, { status: 303, headers: { location: '/gap/', 'set-cookie': cookieHeader(encodeURIComponent(s.value), s.maxAge), 'cache-control': 'no-store' } });
    }
