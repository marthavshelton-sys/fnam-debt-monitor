// Password gate for https://fnam.mx/oracle/* — a Cloudflare Pages Function middleware (same as functions/gap/).
//
// Every request under /oracle/ (the page, its data files, everything) passes through here before
...
//   ORACLE_PASSWORD        the shared password. While it is UNSET the gate is dormant: /oracle/ is served
...
//   ORACLE_SESSION_SECRET  optional. Random string used to sign session cookies. Defaults to a
...
//   ORACLE_SESSION_DAYS    optional. Session length in days (default 30). "?logout" ends a session.
//
// Cloudflare Pages Functions live in /functions at the project root (the directory that holds
// the `site/` build output); the file's path decides the route it guards: functions/oracle/ → /oracle/*.

const COOKIE = 'oracle_session';
const LOGIN_PATH = '/oracle/login';
...
async function sessionSecret(env) {
  return env.ORACLE_SESSION_SECRET || (await sha256Hex('oracle-session:' + env.ORACLE_PASSWORD));
}
async function makeSession(env) {
  const days = Number(env.ORACLE_SESSION_DAYS) > 0 ? Number(env.ORACLE_SESSION_DAYS) : 30;
...
function cookieHeader(value, maxAge) {
  return `${COOKIE}=${value}; Path=/oracle; Max-Age=${maxAge}; HttpOnly; Secure; SameSite=Lax`;
}
...
<p class="fine">Datos públicos (comunicados de resultados, 10-Q y 10-K de Oracle ante la SEC, datos de mercado oficiales). No contiene información privilegiada.</p>`,
  });
}

export async function onRequest({ request, env, next }) {
  const url = new URL(request.url);
  if (!env.ORACLE_PASSWORD) {
    // Password protection not enabled yet: serve the page, keep it out of search engines and caches.
    if (request.method === 'POST' && url.pathname === LOGIN_PATH) return new Response(null, { status: 303, headers: { location: '/oracle/' } });
  ...
  if (url.searchParams.has('logout')) {
    return new Response(null, { status: 303, headers: { location: '/oracle/', 'set-cookie': cookieHeader('', 0), 'cache-control': 'no-store' } });
  }

  if (request.method === 'POST' && url.pathname === LOGIN_PATH) {
    let password = '';
    try { password = String((await request.formData()).get('password') || ''); } catch { password = ''; }
    if (password && timingSafeEqual(password, env.ORACLE_PASSWORD)) {
      const s = await makeSession(env);
      return new Response(null, { status: 303, headers: { location: '/oracle/', 'set-cookie': cookieHeader(encodeURIComponent(s.value), s.maxAge), 'cache-control': 'no-store' } });
    }
