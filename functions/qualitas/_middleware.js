// Password gate for https://fnam.mx/qualitas/* — a Cloudflare Pages Function middleware (same design as
// functions/gap/_middleware.js).
//
// Every request under /qualitas/ (the page, its data files, the quality page) passes through here before
// the static asset is served. Without a valid session cookie the visitor sees a login form; the static
// files are never sent. Sessions are HMAC-signed cookies, so nothing is stored server-side.
//
...
//   QUALITAS_PASSWORD        the shared password. While it is UNSET the gate is dormant: /qualitas/ is
//                            served openly (still with noindex / no-store headers). Setting the variable
//                            and redeploying turns the password on; removing it turns it off again.
//   QUALITAS_SESSION_SECRET  optional. Random string used to sign session cookies. Defaults to a SHA-256
//                            of the password (so changing the password invalidates existing sessions).
//   QUALITAS_SESSION_DAYS    optional. Session length in days (default 30). "?logout" ends a session.
//
// Cloudflare Pages Functions live in /functions at the project root (the directory that holds the `site/`
// build output); the file's path decides the route it guards: functions/qualitas/ → /qualitas/*.

const COOKIE = 'qualitas_session';
const LOGIN_PATH = '/qualitas/login';
const HOME = '/qualitas/';
...
async function sessionSecret(env) {
  return env.QUALITAS_SESSION_SECRET || (await sha256Hex('qualitas-session:' + env.QUALITAS_PASSWORD));
}
async function makeSession(env) {
  const days = Number(env.QUALITAS_SESSION_DAYS) > 0 ? Number(env.QUALITAS_SESSION_DAYS) : 30;
...
function cookieHeader(value, maxAge) {
  return `${COOKIE}=${value}; Path=/qualitas; Max-Age=${maxAge}; HttpOnly; Secure; SameSite=Lax`;
}
...
<p class="fine">Datos públicos (informes trimestrales, reportes SIFIC y eventos relevantes de Quálitas, BMV/CNSF). No contiene información privilegiada.</p>`,
  });
}

export async function onRequest({ request, env, next }) {
  const url = new URL(request.url);
  if (!env.QUALITAS_PASSWORD) {
    // Password protection not enabled yet: serve the page, keep it out of search engines and caches.
    if (request.method === 'POST' && url.pathname === LOGIN_PATH) return new Response(null, { status: 303, headers: { location: HOME } });
  ...
  if (url.searchParams.has('logout')) {
    return new Response(null, { status: 303, headers: { location: HOME, 'set-cookie': cookieHeader('', 0), 'cache-control': 'no-store' } });
  }

  if (request.method === 'POST' && url.pathname === LOGIN_PATH) {
    let password = '';
    try { password = String((await request.formData()).get('password') || ''); } catch { password = ''; }
    if (password && timingSafeEqual(password, env.QUALITAS_PASSWORD)) {
      const s = await makeSession(env);
      return new Response(null, { status: 303, headers: { location: HOME, 'set-cookie': cookieHeader(encodeURIComponent(s.value), s.maxAge), 'cache-control': 'no-store' } });
    }
