...
(function () {
  ...
  function renderMonthly() {
    ...
    const c1 = cn.slice(-12).reverse().map((r) => `<tr><td>${fmtMonth(r.month)}</td><td>${fmtN(r.loans)}</td><td>${fmtPct(r.imor, 2)}</td><td>${fmtN(r.deposits)}</td><td>${fmtN(r.netIncomeYtd)}</td></tr>`).join('');
    const c2 = sb.slice(-12).reverse().map((r) => `<tr><td>${fmtMonth(r.month)}</td><td>${fmtN(r.loans)}</td><td>${fmtPct(r.morosidad, 2)}</td><td>${fmtN(r.deposits)}</td><td>${fmtN(r.writeoffs)}</td><td>${fmtN(r.netIncome)}</td></tr>`).join('');
    html('monthlyTable', `<div class="grid-2 eq" style="margin-top:0">${cn.length ? `<div><table><caption class="small muted" style="text-align:left">${es ? 'CNBV · Banco Compartamos (Ps. M)' : 'CNBV · Banco Compartamos (Ps. M)'}</caption><thead><tr><th>${es ? 'Mes' : 'Month'}</th><th>${t('loans')}</th><th>IMOR</th><th>${es ? 'Captación' : 'Deposits'}</th><th>${es ? 'Resultado acum.' : 'YTD result'}</th></tr></thead><tbody>${c1}</tbody></table></div>` : ''}${sb.length ? `<div><table><caption class="small muted" style="text-align:left">${es ? 'SBS · Compartamos Banco Perú (S/ M)' : 'SBS · Compartamos Banco Perú (S/ M)'}</caption><thead><tr><th>${es ? 'Mes' : 'Month'}</th><th>${t('loans')}</th><th>${es ? 'Morosidad' : 'Delinquency'}</th><th>${es ? 'Depósitos' : 'Deposits'}</th><th>${es ? 'Castigos' : 'Write-offs'}</th><th>${t('netIncome')}</th></tr></thead><tbody>${c2}</tbody></table></div>` : ''}</div>`);
    el('monthlyCap').textContent = es ? 'Últimos 12 meses publicados por cada regulador; cifras en millones de la moneda local.' : 'Last 12 months published by each regulator; figures in millions of local currency.';
