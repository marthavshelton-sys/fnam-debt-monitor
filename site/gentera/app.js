...
(function () {
  ...
  function renderHeader() {
    ...
    if (lastLTM) k.push({ l: `${t('netIncome')} ${t('ltmS')}`, v: 'Ps. ' + fmtN(lastLTM.is.netInc) + ' M', d: `${t('roe')} ${fmtPct(lastLTM.kpi.roe)} · ${lastQ ? qLabel(lastQ) : ''} ${fmtPct(lastQ && lastQ.kpi.roe)}` });
  ...
  function renderMonthly() {
    ...
    const c2 = sb.slice(-12).reverse().map((r) => `<tr><td>${fmtMonth(r.month)}</td><td>${fmtN(r.loans)}</td><td>${fmtPct(r.morosidad, 2)}</td><td>${fmtN(r.deposits)}</td><td>${fmtN(r.writeoffs)}</td><td>${fmtN(r.netIncome)}</td><td>${fmtN(r.equity)}</td></tr>`).join('');
    html('monthlyTable', `<div class="grid-2 eq" style="margin-top:0">${cn.length ? `<div><table><caption class="small muted" style="text-align:left">${es ? 'CNBV · Banco Compartamos (Ps. M; cifras del mes)' : 'CNBV · Banco Compartamos (Ps. M; month-end figures)'}</caption><thead><tr><th>${es ? 'Mes' : 'Month'}</th><th>${t('loans')}</th><th>IMOR</th><th>${es ? 'Cobertura' : 'Coverage'}</th><th>${es ? 'Captación total' : 'Total funding'}</th><th>${es ? 'Activo total' : 'Total assets'}</th><th>ROA</th><th>ROE</th></tr></thead><tbody>${c1}</tbody></table></div>` : ''}${sb.length ? `<div><table><caption class="small muted" style="text-align:left">${es ? 'SBS · Compartamos Banco Perú (S/ M; resultado acumulado en el año)' : 'SBS · Compartamos Banco Perú (S/ M; year-to-date result)'}</caption><thead><tr><th>${es ? 'Mes' : 'Month'}</th><th>${t('loans')}</th><th>${es ? 'Morosidad' : 'Delinquency'}</th><th>${es ? 'Depósitos' : 'Deposits'}</th><th>${es ? 'Castigos' : 'Write-offs'}</th><th>${es ? 'Utilidad acum.' : 'YTD income'}</th><th>${es ? 'Patrimonio' : 'Equity'}</th></tr></thead><tbody>${c2}</tbody></table></div>` : ''}</div>`);
    el('monthlyCap').textContent = es ? 'Últimos 12 meses publicados por cada regulador, en millones de la moneda local. CNBV: cartera total, IMOR y cobertura del Boletín Estadístico (captación total = depósitos + préstamos interbancarios + títulos). SBS: balance y morosidad mensuales; Compartamos Banco figura en Banca Múltiple desde 2025.' : 'Last 12 months published by each regulator, in millions of local currency. CNBV: total loans, IMOR and coverage from the statistical bulletin (total funding = deposits + interbank loans + securities). SBS: monthly balance sheet and delinquency; Compartamos Banco appears under Banca Múltiple from 2025.';
  ...
  function renderRelative() {
    ...
    html('multSrc', `${t('src')}: Yahoo Finance${qPx.length ? '' : ' (' + t('pendingMk') + ')'} · ${es ? 'informes trimestrales (capital y utilidad controladores)' : 'quarterly releases (controlling equity and income)'}`);
    const P = PEERS.peers || [];
    const cell = (v, f) => (v == null ? `<span class="muted">${t('pending')}</span>` : f(v));
  ...
  function renderAssetQuality() {
    ...
    html('nplSrc', `${t('src')}: ${es ? 'informes trimestrales (indicadores consolidados y secciones por subsidiaria)' : 'quarterly releases (consolidated indicators and subsidiary sections)'}`);
    ...
    html('corSrc', `${t('src')}: ${es ? 'castigos de la tabla de cartera de cada informe (Banco = consolidado − Perú − ConCrédito); costo de riesgo y cobertura recalculados' : 'write-offs from the portfolio table of each release (Bank = consolidated − Perú − ConCrédito); cost of risk and coverage recomputed'}`);
    ...
    html('fundSrc', `${t('src')}: ${es ? 'balance general de cada informe' : 'balance sheet of each release'}`);
    ...
    html('capSrc', `${t('src')}: ${es ? 'secciones de Banco Compartamos y Compartamos Banco Perú de cada informe; capital ÷ activos del balance consolidado' : 'Banco Compartamos and Compartamos Banco Perú sections of each release; equity ÷ assets from the consolidated balance sheet'}`);
    const q8 = Q.slice(-8);
    const rowsDef = [
  ...
  function renderDividends() {
    ...
    html('dpsSrc', `${t('src')}: ${es ? 'dividendos aprobados en cada asamblea anual (informes trimestrales de Gentera); por acción = monto ÷ acciones en circulación del año' : 'dividends approved at each annual meeting (Gentera quarterly releases); per share = amount ÷ that year\'s shares outstanding'}`);
  ...
  function renderSpecial() {
    ...
    facts('ccFacts', CC.facts); tl('ccTimeline', CC.timeline); html('ccSrc', `${t('src')}: ${(CC.sources || []).join(' · ')}`);
    facts('peFacts', PE.facts); tl('peTimeline', PE.timeline); html('peSrc', `${t('src')}: ${(PE.sources || []).join(' · ')}`);
    facts('glFacts', GL.facts); html('glSrc', `${t('src')}: ${(GL.sources || []).join(' · ')}`);
  }
