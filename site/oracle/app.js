...
(function () {
  ...
  function renderBuildout() {
    ...
    const srcShort = (r) => { const label = r.short || r.title; return r.url ? `<a href="${r.url}" target="_blank" rel="noopener" title="${(r.title || '').replace(/"/g, '&quot;')}">${label}</a>` : `<span title="${(r.title || '').replace(/"/g, '&quot;')}">${label}</span>`; };
    html('sitesTable', `<table class="sites"><thead><tr><th>${LANG === 'es' ? 'Sitio' : 'Site'}</th><th>${LANG === 'es' ? 'Capacidad' : 'Capacity'}</th><th>${LANG === 'es' ? 'Cliente · desarrollador · financiamiento' : 'Customer · developer · financing'}</th><th>${LANG === 'es' ? 'Estado según Oracle' : 'Oracle\'s status'}</th><th>${t('src')}</th></tr></thead><tbody>${sites.map((s) => `<tr><td><b>${s.name}</b><span class="sub">${s.location}</span></td><td><b>${s.capacity_mw ? fmtN(s.capacity_mw) + ' MW' : '—'}</b><span class="sub">${s.capacity_text || ''}</span></td><td><b>${s.customer || '—'}</b><span class="sub">${s.developer || '—'}</span><span class="sub">${s.financing || ''}</span></td><td>${s.oracle_status || ''}<span class="sub">${LANG === 'es' ? 'Contratado' : 'Contracted'}: ${s.contracted || '—'}</span><span class="sub">${LANG === 'es' ? 'Primera entrega' : 'First delivery'}: ${s.first_delivery || '—'}</span></td><td class="small">${(s.sources || []).map(srcShort).join(' · ')}</td></tr>`).join('')}</tbody></table>`);
