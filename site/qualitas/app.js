...
(function () {
  ...
  function renderGuidance() {
    ...
    html('guideCurSrc', `${t('src')}: <a href="${cur.source.url}" target="_blank" rel="noopener">${L(cur.source.title)} (${fmtDate(cur.date)}) ↗</a> · ${LANG === 'es' ? 'Crecimiento sobre el año fiscal anterior en %, índices como nivel en %. Siniestralidad = costo de siniestros ÷ prima devengada; combinado = adquisición (sobre retenida) + siniestralidad (sobre devengada) + operación (sobre emitida), definiciones de Quálitas.' : (GD.basis || '')}${asOfFile(GD, 'data/guidance.js')}`);
